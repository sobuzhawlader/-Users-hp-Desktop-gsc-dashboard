...
# Email Configuration
# ==============================
EMAIL_SENDER = ""      # আপনার Gmail এখানে দিন
EMAIL_PASSWORD = ""    # Gmail App Password এখানে দিন
...
class GSCReport(FPDF):
    def header(self):
        ...
        self.cell(0, 15, 'GSC Pro Dashboard - Monthly Report', 0, 1, 'C')
    ...
    def add_kpi_row(self, label, value, change=None):
        ...
        self.cell(80, 8, label, 1, 0, 'L')
        self.set_font('Arial', 'B', 11)
        self.cell(50, 8, str(value), 1, 0, 'C')
...
def generate_pdf_report(site_url, overview, top_keywords, top_pages, 
    ...
    if filename is None:
        filename = f"GSC_Report_{datetime.now().strftime('%Y_%m')}.pdf"
    ...
    pdf.cell(0, 8, f"Site: {site_url}", 0, 1, 'L', True)
...
def send_email_report(recipient_email, site_url, pdf_path):
    ...
    try:
        ...
        msg['Subject'] = f"GSC Monthly Report - {site_url} - {datetime.now().strftime('%B %Y')}"
